...
# Set up argument parsing for cloth image
...
if cloth_img is None:
    print(json.dumps({"type": "error", "message": "Could not read cloth image from path: " + args.cloth_image}))
    sys.stdout.flush()
    sys.exit(1)
...
def overlay_cloth(frame, upper_body_rect, cloth_img, cloth_mask):
    ...
    # Ensure cloth_img and cloth_mask are resized to match roi dimensions
    # This might be redundant if process_cloth_image already handles the scaling to target_width/height correctly for the ROI size
    ...
    while True:
        ...
        try:
            ...
            if input_data.get("type") == "frame_input":
                frame_base64_input = input_data.get("data")
                nparr = np.frombuffer(base64.b64decode(frame_base64_input), np.uint8)
                frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR)

                if frame is None:
                    print(json.dumps({"type": "error", "message": "Failed to decode received frame."}))
                    sys.stdout.flush()
                    continue

                frame = cv2.flip(frame, 1) # Still flip horizontally for mirror effect
                ...
                # Encode processed frame to JPEG and base64
                ret, buffer = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 70])
                if not ret:
                    print(json.dumps({"type": "error", "message": "Failed to encode processed frame."}))
                    sys.stdout.flush()
                    continue

                frame_base64_output = base64.b64encode(buffer).decode('utf-8')

                # Send processed frame as JSON
                print(json.dumps({
                    "type": "frame",
                    "data": frame_base64_output,
                    "timestamp": time.time()
                }))
                sys.stdout.flush()
            elif input_data.get("type") == "command" and input_data.get("action") == "stop":
                print(json.dumps({"type": "status", "message": "Stop command received, exiting."}))
                sys.stdout.flush()
                break # Exit the loop on stop command
            else:
                print(json.dumps({"type": "status", "message": "Received unknown JSON input type."}))
                sys.stdout.flush()

        except json.JSONDecodeError:
            print(json.dumps({"type": "error", "message": "Invalid JSON input: " + line.strip()}))
            sys.stdout.flush()
        except Exception as e:
            print(json.dumps({"type": "error", "message": f"An unexpected error occurred: {e}"}))
            sys.stdout.flush()

except KeyboardInterrupt:
    print(json.dumps({"type": "status", "message": "KeyboardInterrupt received, stopping."}))
    sys.stdout.flush()
finally:
