...
def detect_upper_body(frame):
    ...
    if len(faces) == 0:
        ...
        upper_body_rect = (
            ...
            y + int(h * 1.5)  # Extend further down the body
        )
    else:
        (x, y, w, h) = faces[0]
        upper_bodies = upper_body_cascade.detectMultiScale(
        ...
        if len(upper_bodies) > 0:
            ...
            upper_body_rect = (
                ...
                y + h,  # Start from bottom of face (neck position)
                min(frame.shape[1]-1, ub_x + ub_w + int(ub_w * width_expansion)),
                ub_y + int(ub_h * 1.5)  # Extend further down the body
            )
        else:
            neck_y = y + h
            shoulder_width = int(w * 3.5)  # Increased shoulder width
            left_shoulder_x = max(0, x - int((shoulder_width - w) / 2))
            right_shoulder_x = min(frame.shape[1]-1, left_shoulder_x + shoulder_width)
            hips_y = neck_y + int(h * 3.5)  # Extended further down
            upper_body_rect = (
                left_shoulder_x,
                neck_y,
                right_shoulder_x,
...
def process_cloth_image(cloth_img, target_width, target_height):
    ...
    # Calculate scale factors to fill the target area
    scale_width = target_width / w
    scale_height = target_height / h
    effective_scale = max(scale_width, scale_height) * 1.1  # Slightly larger than target
    ...
    # Center the cloth horizontally and position it starting from the top
    x_offset = max(0, (target_width - new_w) // 2)
    y_offset = 0  # Start from top of the ROI
